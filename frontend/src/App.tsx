...
const categoryColor = (category: string) => categoryColors[Math.max(category.length - 1, 0) % categoryColors.length];
...
function App() {
  ...
  const categories = useMemo<CategoryItem[]>(
    () => Array.from(new Set(places.map((place) => place.category))).map((name) => ({
      name,
    ...
    () =>
      places.filter(
        (p) =>
          ...
          `${p.name} ${p.formatted_address ?? ""} ${p.category}`
            .toLowerCase()
        ...
        <section className="service-popup">
          ...
          <div className="service-title">
            <span
              ...
              style={{
                background: categories.find((c) => c.name === selected.category)
                  ?.color,
              }}
            >
          ...
          <div className="service-info">
            <p>
              <MapPinned size={17} />
              <span>{selected.formatted_address ?? "Address unavailable"}</span>
            </p>
          ...
          <div className="service-actions">
            <button
              onClick={() =>
                setNotice(`${selected.name} uses ${selected.sourcename} data.`)
              }
            >
              View Details
            </button>
